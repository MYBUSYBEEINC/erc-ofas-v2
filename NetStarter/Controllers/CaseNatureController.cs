...
namespace NetStarter.Controllers
{
    ...
    public class CaseNatureController : Controller
    {
        ...
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.CaseNature, "true", "", "", "")]
        public ViewResult Index()
        {
            return View();
        }

        public ActionResult GetPartialViewCaseNatures()
        {
            CaseNatureListing caseNatureListing = new CaseNatureListing();
            caseNatureListing.Listing = ReadCaseNatures();
            return PartialView("~/Views/CaseNature/_MainList.cshtml", caseNatureListing);
        }

        public List<GlobalOptionSetViewModel> ReadCaseNatures()
        {
            List<GlobalOptionSetViewModel> result = new List<GlobalOptionSetViewModel>();
            result = (from t1 in db.GlobalOptionSets
                      join t2 in db.GlobalOptionSets on t1.ReferenceId equals t2.Id into gj
                      from t2X in gj.DefaultIfEmpty()
                      where t1.Type == "CaseNature" && t1.Status == "Active"
                      select new GlobalOptionSetViewModel
                      {
                          Id = t1.Id,
                          Code = t1.Code,
                          DisplayName = t1.DisplayName,
                          ReferenceId = t2X == null ? "" : t2X.DisplayName,
        ...
        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.CaseNature, "", "true", "true", "")]
        public ActionResult Edit(string Id)
        {
            ...
            {
                //display order
                int? maxOrder = db.GlobalOptionSets.Where(a => a.Type == "CaseNature" && a.Status == "Active").Select(a => a.OptionOrder).OrderByDescending(a => a.Value).FirstOrDefault();
                model.OptionOrder = maxOrder + 1;
            }
            model.ReferenceSelectList = general.GetCaseTypeList(model.ReferenceId);
            return View(model);
        }
        ...
        public void ValidateModel(GlobalOptionSetViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    duplicated = db.GlobalOptionSets.Where(a => a.DisplayName == model.DisplayName && a.Type == "CaseNatures" && a.Id != model.Id).Any();
                }
                else
                {
                    duplicated = db.GlobalOptionSets.Where(a => a.DisplayName == model.DisplayName && a.Type == "CaseNatures").Select(a => a.Id).Any();
                }
        ...
        public void SaveRecord(GlobalOptionSetViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Id != null)
                {
                    ...
                    globalOptionSet.Type = "CaseNature";
