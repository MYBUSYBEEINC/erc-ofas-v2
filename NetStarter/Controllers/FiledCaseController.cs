...
{
    [Authorize]
    public class FiledCaseController : Controller
    {
        private DefaultDBContext db = new DefaultDBContext();
        private GeneralController general = new GeneralController();

        [CustomAuthorizeFilter(ProjectEnum.ModuleCode.FiledCase, "true", "", "", "")]
        public ViewResult Index()
        {
            return View();
        }

        public ActionResult GetPartialViewFiledCases()
        {
            PreFiledCaseListing theListing = new PreFiledCaseListing();
            theListing.Listing = ReadFiledCases();
            return PartialView("~/Views/FiledCase/_MainList.cshtml", theListing);
        }
